...
abstract class BaseCargo extends BaseObject  implements Persistent
{
	...
	public function setIdCargo($v)
	{
		...
		if ($this->id_cargo !== $v) {
			$this->id_cargo = $v;
			$this->modifiedColumns[] = CargoPeer::ID_CARGO;
		}
	...
	public function setEstado($v)
	{
		...
		if ($this->estado !== $v) {
			$this->estado = $v;
			$this->modifiedColumns[] = CargoPeer::ESTADO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = CargoPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CargoPeer::ID_CARGO) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CargoPeer::ID_CARGO)) $criteria->add(CargoPeer::ID_CARGO, $this->id_cargo);
		if ($this->isColumnModified(CargoPeer::DESCRIPCION)) $criteria->add(CargoPeer::DESCRIPCION, $this->descripcion);
		if ($this->isColumnModified(CargoPeer::ESTADO)) $criteria->add(CargoPeer::ESTADO, $this->estado);
	...
	{
		$criteria = new Criteria(CargoPeer::DATABASE_NAME);
		$criteria->add(CargoPeer::ID_CARGO, $this->id_cargo);
