...
abstract class BaseFormaPago extends BaseObject  implements Persistent
{
	...
	public function setDescripcion($v)
	{
		...
		if ($this->descripcion !== $v) {
			$this->descripcion = $v;
			$this->modifiedColumns[] = FormaPagoPeer::DESCRIPCION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = FormaPagoPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FormaPagoPeer::ID_FORMA_PAGO) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.FormaPagoPeer::ID_FORMA_PAGO.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setIdFormaPago($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FormaPagoPeer::DESCRIPCION)) $criteria->add(FormaPagoPeer::DESCRIPCION, $this->descripcion);
